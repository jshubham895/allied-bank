...
const Account = () => {
	...
	const loadAccount = async () => {
		try {
			await Axios.get(
				`https://us-central1-allied-bank-2788f.cloudfunctions.net/app/accounts/${accountId}`,
				config
			).then((result) => {
	...
	const loadAccounts = async () => {
		const result = await Axios.get(
			`https://us-central1-allied-bank-2788f.cloudfunctions.net/app/accounts`,
			config
		);
	...
	const transactions = async () => {
		const result = await Axios.get(
			`https://us-central1-allied-bank-2788f.cloudfunctions.net/app/transactions/account/${accountId}`,
			config
		);
	...
	const onSubmit = async (e) => {
		...
		try {
			const response = await Axios.post(
				"https://us-central1-allied-bank-2788f.cloudfunctions.net/app/transactions",
				{
					from: account.name,
					to: to,
					fromId: fromId,
